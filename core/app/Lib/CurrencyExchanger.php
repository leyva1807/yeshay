...
class CurrencyExchanger
{
    ...
    public function currencyExchanger(Request $request)
    {
        ...
        if ($this->sendAmount < $this->sendCurrency->minimum_limit_for_buy) {
            $notify[] = ['error', "Minimum sending amount " . showAmount($this->sendCurrency->minimum_limit_for_buy, currencyFormat: false) . ' ' . $this->sendCurrency->cur_sym];
            return [
                'status' => 'error',
                'notify' => $notify
            ];
        ...
        if ($this->sendAmount > $this->sendCurrency->maximum_limit_for_buy) {
            $notify[] = ['error', "Maximum sending amount " . showAmount($this->sendCurrency->maximum_limit_for_buy, currencyFormat: false) . ' ' . $this->sendCurrency->cur_sym];
            return [
                'status' => 'error',
                'notify' => $notify
            ];
        ...
        if ($this->receiveAmount < $this->receiveCurrency->minimum_limit_for_sell) {
            $notify[] = ['error', "Minimum received amount " . showAmount($this->receiveCurrency->minimum_limit_for_sell, currencyFormat: false) . ' ' . $this->receiveCurrency->cur_sym];
            return [
                'status' => 'error',
                'notify' => $notify
            ];
        ...
        if ($this->receiveAmount > $this->receiveCurrency->maximum_limit_for_sell) {
            $notify[] = ['error', "Maximum received amount " . showAmount($this->receiveCurrency->maximum_limit_for_sell, currencyFormat: false) . ' ' . $this->receiveCurrency->cur_sym];
            return [
                'status' => 'error',
                'notify' => $notify
            ];
